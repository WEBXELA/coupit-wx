    ...
    <section className="bg-[#f7f7f7] section-padding">
      <div className="max-w-6xl mx-auto">
        <div className="grid md:grid-cols-4 gap-8">
          {[
            { icon: <Users className="w-8 h-8 text-[#2B2C30]" />, stat: "25K+", label: "UK Active Users" },
            { icon: <Store className="w-8 h-8 text-[#2B2C30]" />, stat: "500+", label: "UK High Street Stores" },
            { icon: <TrendingUp className="w-8 h-8 text-[#2B2C30]" />, stat: "35%", label: "Footfall Increase" },
            { icon: <ShoppingBag className="w-8 h-8 text-[#2B2C30]" />, stat: "2.8x", label: "Average ROI" }
          ].map((item, index) => (
            <div 
              key={index} 
            ...
            >
              <div className="relative z-10">
                ...
                <p className="text-3xl font-bold mt-4 mb-2 text-[#2B2C30] 
